...
protected:
  ...
  tvm::ffi::ObjectPtr<SimpleAttentionKVCacheObj> CreateSimpleKVCache() {
    return tvm::ffi::make_object<SimpleAttentionKVCacheObj>(
        ...
        RoPEMode::kNone, 1.0, 10000.0, std::nullopt,
        dtype_, device_, f_append_kv_, f_attention_prefill_, f_attention_decode_);
  }
...
TEST_F(SimpleKVCacheTest, AttentionWithFusedQKV) {
  ...
  // Create test data
  NDArray qkv_data = NDArray::Empty(
      {total_len, num_qo_heads_ + 2 * num_kv_heads_, head_dim_}, dtype_, device_);
  NDArray output = NDArray::Empty(
      {total_len, num_qo_heads_, head_dim_}, dtype_, device_);
...
TEST_F(SimpleKVCacheTest, AttentionWithInvalidLayer) {
  ...
  NDArray qkv_data = NDArray::Empty({1, num_qo_heads_ + 2 * num_kv_heads_, head_dim_}, dtype_, device_);
  NDArray output = NDArray::Empty({1, num_qo_heads_, head_dim_}, dtype_, device_);
...
TEST_F(SimpleKVCacheTest, SelfAttention) {
  ...
  // Create test data
  NDArray q_data = NDArray::Empty({total_len, num_qo_heads_, head_dim_}, dtype_, device_);
  NDArray k_data = NDArray::Empty({total_len, num_kv_heads_, head_dim_}, dtype_, device_);
  NDArray v_data = NDArray::Empty({total_len, num_kv_heads_, head_dim_}, dtype_, device_);
  NDArray output = NDArray::Empty({total_len, num_qo_heads_, head_dim_}, dtype_, device_);
  NDArray lse_data = NDArray::Empty({total_len, num_qo_heads_}, dtype_, device_);
...
TEST_F(SimpleKVCacheTest, GetQueryPositions) {
  ...
  NDArray positions = cache_->GetQueryPositions();
...
TEST_F(SimpleKVCacheTest, DebugGetKV) {
  ...
  int64_t start_pos = 0;
  int64_t end_pos = 5;
  NDArray k_data = NDArray::Empty(
      {num_layers_, end_pos - start_pos, num_kv_heads_, head_dim_}, dtype_, device_);
  NDArray v_data = NDArray::Empty(
      {num_layers_, end_pos - start_pos, num_kv_heads_, head_dim_}, dtype_, device_);
